...
void test_corrections_TAE_setup()
{
  ...
  configPage4.aeValues[0] = 70;
  configPage4.aeValues[1] = 103; 
  configPage4.aeValues[2] = 124;
  configPage4.aeValues[3] = 136; 

  //Note: These values are divided by 10
  configPage4.aePosBins[0] = 0;
  configPage4.aePosBins[1] = 8; 
  configPage4.aePosBins[2] = 22;
  configPage4.aePosBins[3] = 97; 
  
  configPage2.aeThresh = 0;
...
void test_corrections_TAE_no_rpm_taper()
{
  ...
  TEST_ASSERT_EQUAL(750, currentStatus.aeChangeRate); //DOT is 750%/s (25 * 30)
...
void test_corrections_TAE_under_threshold()
{
  ...
  currentStatus.TPS = 6; //3% actual value. TPSDot should be 90%/s
	configPage2.aeThresh = 100; //Above the reading of 90%/s

  uint16_t accelValue = correctionAccel(); //Run the AE calcs

  TEST_ASSERT_EQUAL(90, currentStatus.aeChangeRate); //DOT is 90%/s (3% * 30)
  TEST_ASSERT_EQUAL(100, accelValue); //Should be no AE as we're above the RPM taper end point
